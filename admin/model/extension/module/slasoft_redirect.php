<?php
class ModelExtensionModuleSlasoftRedirect extends Model {

    public function clear() {
        $sql = "TRUNCATE `" . DB_PREFIX . "slasoft_redirect`";
		$this->db->query($sql);
	}

    public function getTotalRules($filter_data = array()) {
        $sql = "SELECT COUNT(*) total FROM `" . DB_PREFIX . "slasoft_redirect` p where 1 ";
    ...
    public function checkFromUrl($from_url) {
        $sql = "SELECT * from `" . DB_PREFIX . "slasoft_redirect` p where from_url = '" . $this->db->escape($from_url) . "'";
		$result = $this->db->query($sql);
		return $result->row;
	}
	
    public function getRules($filter_data = array()) {
        $sql = "SHOW TABLES LIKE '" . DB_PREFIX . "slasoft_redirect'";
		$result = $this->db->query($sql);
		if (!$result->num_rows) {
			$this->install();
		}

        $sql = "SELECT * from `" . DB_PREFIX . "slasoft_redirect` p where 1 ";
    ...
    public function addRule($data) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "slasoft_redirect` WHERE `from_url` = '" . ltrim($this->db->escape($data['from_url']),'/') . "'");
		$sql = "INSERT INTO `" . DB_PREFIX . "slasoft_redirect` SET
    ...
    public function getRule($redirect_id) {
		$sql = "SELECT * FROM `" . DB_PREFIX . "slasoft_redirect`
			WHERE `redirect_id` = " . (int)$redirect_id;
		$result = $this->db->query($sql);
		return $result->row;
    }
    ...
    public function deleteRule($redirect_id) {
        $this->db->query("DELETE FROM `" . DB_PREFIX . "slasoft_redirect` where `redirect_id` = " . (int)$redirect_id);
		$this->cache->delete('redirect');
    }

    public function uninstall() {
		$sql = "DROP TABLE IF EXISTS `" . DB_PREFIX . "slasoft_redirect`";
		$this->db->query($sql);
		$this->cache->delete('redirect');
	}

    public function install() {
		$sql = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "slasoft_redirect` (
		...
		$this->db->query($sql);
		$this->db->query("ALTER TABLE `" . DB_PREFIX . "slasoft_redirect` CHANGE `from_url` `from_url` VARCHAR(512) NULL DEFAULT NULL");
		$this->db->query("ALTER TABLE `" . DB_PREFIX . "slasoft_redirect` DROP INDEX `from_url`");
		$this->cache->delete('redirect');
	}
