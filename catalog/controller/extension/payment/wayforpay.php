...
class ControllerExtensionPaymentWayforpay extends Controller
{
	...
	public function generateFields() {
        ...
        foreach ($products as $product) {
            // Отримуємо альтернативну назву
            $alternative_data = $this->model_extension_payment_wayforpay_products->getAlternativeProductName($product['product_id']);
            
            // Використовуємо альтернативну назву якщо вона є, якщо немає - оригінальну
            $product_name = ($alternative_data && isset($alternative_data['alternative_name'])) 
                ? $alternative_data['alternative_name'] 
                : $product['name'];
            
            // Очищуємо назву від спеціальних символів і обмежуємо довжину до 255 символів
            $cleaned_name = str_replace(
                array("'", '"', '&#39;', '&'), 
                '', 
                htmlspecialchars_decode($product_name)
            );
            $cleaned_name = mb_substr($cleaned_name, 0, 255);
            
            $productNames[] = $cleaned_name;
