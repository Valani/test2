...
class ControllerCheckoutOctSmartcheckout extends Controller {
    public function index() {
        ...
        $this->document->addStyle('catalog/view/theme/oct_showcase/stylesheet/smartcheckout.css');
    ...
    private function getRecommendedProducts() {
        ...
        if (isset($oct_smart_checkout_data['recommended_poducts']['status']) && $oct_smart_checkout_data['recommended_poducts']['status'] == "on" && $oct_smart_checkout_data['recommended_poducts']['products']) {
            $product_ids = $oct_smart_checkout_data['recommended_poducts']['products'];
            $products = $this->cart->getProducts();
    ...
    private function getCustomFieldOptions($customFieldId, $languageId) {
        $query = $this->db->query("SELECT cfv.custom_field_value_id, cfvd.name FROM " . DB_PREFIX . "custom_field_value cfv LEFT JOIN " . DB_PREFIX . "custom_field_value_description cfvd ON (cfv.custom_field_value_id = cfvd.custom_field_value_id) WHERE cfv.custom_field_id = '" . (int)$customFieldId . "' AND cfvd.language_id = '" . (int)$languageId . "'");
    ...
    private function shippingMethods($render = false, &$data = array()) {
        ...
        if (isset($country_id)) {
            ...
            $shipping_address_id = (isset($this->request->post['address_id']) ? $this->request->post['address_id'] : (isset($this->session->data['address_id']) ? $this->session->data['address_id'] : null));
                        ...
                        foreach ($quote['quote'] as $quote_code => $quote_info) {
                            ...
                            if (isset($oct_shipping_settings[$shippingQuoteCode])) {
                                ...
                                if (isset($delivery_setting['status']) && $delivery_setting['status'] == 'on') {
                                    $mainImage = isset($delivery_setting['image']) ? $delivery_setting['image'] : '';
            ...
            if ($this->config->get('payment_' . $result['code'] . '_status')) {
                ...
                if (isset($oct_payment_settings[$result['code']]) && $method) {
                    ...
                    if (isset($payment_setting['local_edit'])) {
                        $method['description'] = $localization['description'] ?? '';
            ...
            foreach ($data['merged_fields'] as $key => $field) {
                if (isset($field['merge']) && $field['merge'] == 'on' && isset($field['merge_field'])) {
                    ...
                    if (isset($this->request->post[$key])) {
                        ...
                        $fieldValue = $fieldName . ": " . $this->request->post[$key];
            ...
            switch ($fieldName) {
                ...
                case 'telephone':
                    ...
                    if ($field_settings['required'] && !empty($postedTelephone)) {
                        ...
                        } elseif ($cleanPostedTelephone > 15 || $cleanPostedTelephone < 3) {
                            $json['error']['telephone'] = $this->getErrorMessage($field_settings, $fieldName);
                        }
                    }
                    break;
